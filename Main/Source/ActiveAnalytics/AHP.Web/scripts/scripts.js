$(window).load(function () {
    ...
    if ($.browser.msie) {
        $(".menu_pull_up").removeClass("menu_pull_up");
    }

    if ($.browser.msie && $.browser.version < 9)
        $(window).resize(function () {
            glob.height($(window).height() < 740 ? '740px' : '100%')
        });
    ...
    nav
        ...
        .navs(function (n, _) {
            ...
            analyticReporter.report = $.noop;
            if (window.ADRUM && window.ADRUM.report) {
                analyticReporter = window.ADRUM;
                tracker = new ADRUM.events.VPageView(window.location.protocol + "//" + location.hostname + location.pathname + n);
            }
            tracker.url(window.location.protocol + "//" + location.hostname + location.pathname + n);
            tracker.start();
            if (n === _.defHash)
                show_splash();
            else
            ...
            content.tabs(n);
            tracker.end();
            analyticReporter.report(tracker);
        })
